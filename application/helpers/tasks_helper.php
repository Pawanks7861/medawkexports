...
function get_lead_status_by_lead_id($lead_id) {
    ...
    $outputStatus = '<span class="lead-status-' . $lead->status . ' label' . (empty($lead->color) ? ' label-default' : '') . '" 
                      style="color:' . $lead->color . ';border:1px solid ' . adjust_hex_brightness($lead->color, 0.4) . ';background: ' . adjust_hex_brightness($lead->color, 0.04) . ';">' 
                    . e($lead->lead_status) . '</span>';

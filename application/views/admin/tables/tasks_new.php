...
// ============ MODIFIED: Staff can only see their assigned tasks ============
...
if (!is_admin()) {
    ...
    $where[] = 'AND (
        ' . db_prefix() . 'tasks.id IN (
            SELECT taskid 
            FROM ' . db_prefix() . 'task_assigned 
            WHERE staffid = ' . $current_staff_id . '
        )
        OR ' . db_prefix() . 'tasks.addedfrom = ' . $current_staff_id . '
    )';
}

// Keep the existing permission check for non-admin users (but now it's redundant with the above)
if (staff_cant('view', 'tasks')) {
    // This will now only apply to non-admin users who don't have view permission
    // The condition above already restricts to assigned tasks

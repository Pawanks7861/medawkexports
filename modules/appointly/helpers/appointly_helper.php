...
if (in_array('dom', get_loaded_extensions()) && get_instance()->app->get_current_db_version() >= 280) {
    ...
    function domCreateNewElement($row, $data, $title = '', $text = '')
    {
        ...
        // FIX: Correctly find the div with class "row-options"
        $rowOptions = null;
        $divs = $doc->getElementsByTagName('div');
        
        // Loop through all divs to find the one with class "row-options"
        foreach ($divs as $div) {
            $class = $div->getAttribute('class');
            if (strpos($class, 'row-options') !== false) {
                $rowOptions = $div;
                break;
            }
        }
        
        // If row-options div not found, check for has-row-options
        if ($rowOptions === null) {
            foreach ($divs as $div) {
                $class = $div->getAttribute('class');
                if (strpos($class, 'has-row-options') !== false) {
                    $rowOptions = $div;
                    break;
                }
            }
        ...
        // If no suitable div found, create one
        if ($rowOptions === null) {
            $rowOptions = $doc->createElement('div');
            $rowOptions->setAttribute('class', 'row-options');
            // Append to body or appropriate parent
            $body = $doc->getElementsByTagName('body')->item(0);
            if ($body) {
                $body->appendChild($rowOptions);
            }
        }

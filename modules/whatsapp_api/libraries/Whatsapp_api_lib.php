...
class Whatsapp_api_lib
{
    ...
    public function send_mapped_template($category, $return=false, $request_from="MANUAL", ...$params)
    {
        ...
        foreach ($all_templates as $template) {
            ...
            if ($category == "secure_login" && !empty(json_decode($template->buttons_data))) {
                $this->send_data['template']['components'][4] = ['type' => "button", 'sub_type'=> "url", "index" => 0];
                $this->send_data['template']['components'][4]['parameters'][] = ['type' => 'text', 'text' => $staff->whatsapp_auth_code];
            }

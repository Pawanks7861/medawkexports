...
class Whatsapp_api extends AdminController
{
    ...
    public function broadcast_messages()
    {
        ...
        if ($this->input->post()) {
            ...
            if (!empty($_FILES['image']['name'])) {
                ...
                if ($image_name) {
                    $image_url = base_url("uploads/whatsapp_api/broadcast_images/" . $image_name);
                }
            }
            ...
            $this->whatsapp_api_lib->send_custom_message($receiverData, $post_data, $image_url);
            echo json_encode([
                'success' => true,
            ]);
            die();
    ...
    public function whatsapp_cron_job_queue_table()
    {
        if (!$this->input->is_ajax_request()) {
            ajax_access_denied();
        }

        $this->app->get_table_data(module_views_path(WHATSAPP_API_MODULE, 'tables/whatsapp_cron_job_queue_table'));
    }

    public function delete_scheduled_call($id): void
    {
        if ($this->whatsapp_api_model->delete_scheduled_call(['id' => $id])) {
            set_alert('success', _l('deleted', 'whatsapp_cron_queue'));
        } else {
            set_alert('danger', _l('problem_deleting', 'whatsapp_cron_queue'));
        }

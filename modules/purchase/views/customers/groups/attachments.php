...
<hr />
<?php if(isset($client)){ ?>
    <?php echo form_open_multipart(admin_url('purchase/upload_customer_attachment/'.$client->userid),array('class'=>'dropzone','id'=>'client-attachments-upload')); ?>
                        ...
                        <?php
                            ...
                            if(count($attachments) > 0){
                                ...
                                foreach ($attachments as $f) {
                                    $href_url = site_url(PURCHASE_PATH.'pur_customer/'.$f['rel_id'].'/'.$f['file_name']).'" download';
                                                    if(!empty($f['external'])){

...
$hasPermissionDelete = has_permission('purchase_customers', '', 'delete');
...
foreach ($rResult as $key => $aRow) {
    ...
    $url = admin_url('purchase/customer/' . $aRow['userid']);
    ...
    if (!$isPerson) {
        $company .= ' | <a href="' . admin_url('purchase/customer/' . $aRow['userid'] . '?group=contacts') . '">' . _l('customer_contacts') . '</a>';
    }
    if ($hasPermissionDelete) {
        $company .= ' | <a href="' . admin_url('purchase/delete_customer/' . $aRow['userid']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    }

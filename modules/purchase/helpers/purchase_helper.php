...
function get_sql_select_customer_company()
{
    return 'CASE company WHEN "" THEN (SELECT CONCAT(firstname, " ", lastname) FROM ' . db_prefix() . 'pur_customer_contacts WHERE ' . db_prefix() . 'pur_customer_contacts.userid = ' . db_prefix() . 'pur_customer.userid and is_primary = 1) ELSE company END as company';
}
...
function get_all_pur_customer_attachments($id)
{
    $CI = &get_instance();
    ...
    $CI->db->where('rel_id', $id);
    $CI->db->where('rel_type', 'pur_customer');
    $client_main_attachments = $CI->db->get(db_prefix() . 'files')->result_array();
...
function format_pdf_vendor_info($vendor_id)
{
    ...
    function handle_pur_customer_attachments_upload($id, $customer_upload = false)
    {

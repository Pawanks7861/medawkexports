    ...
    if (app.options.company_is_required == 1) {
        vRules = {
            ...
            vendor_code: {
               required: true,
               remote: {
                url: site_url + "admin/purchase/customer_code_exists",
                type: 'post',
                data: {
...
function view_ic_file(id, rel_id) {
    "use strict";
      $('#ic_file_data').empty();
      $("#ic_file_data").load(admin_url + 'purchase/file_pur_customer/' + id + '/' + rel_id, function(response, status, xhr) {
          if (status == "error") {
...
function delete_ic_attachment(id) {
    "use strict";
    if (confirm_delete()) {
        requestGet('purchase/delete_pc_attachment/' + id).done(function(success) {
            if (success == 1) {
